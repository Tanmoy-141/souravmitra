...
import { verifySessionWithRevocationCheck } from "@/lib/auth";

/**
 * Validates the cryptographic session token against tampering, forgery, expiration, and active revocation.
 */
async function isAuthorized(req?: NextRequest): Promise<boolean> {
  ...
  if (sessionCookie) {
    const result = await verifySessionWithRevocationCheck(sessionCookie);
    if (result.valid) {
      return true;
    }
  }
  ...
  if (req) {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      const result = await verifySessionWithRevocationCheck(token);
      if (result.valid) {
        return true;
      }
    }
